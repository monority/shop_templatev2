  ...
  return (
    <div className="min-h-screen bg-light">
      ...
      <div className="container max-w-4xl mx-auto px-6 py-20 space-y-12">
        {/* How to measure */}
        <section>
          <h2 className="text-3xl font-bold text-dark mb-6">How to Measure Your Foot</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <div key={step} className="card p-6">
                <div className="w-10 h-10 bg-brand text-white rounded-full flex items-center justify-center font-extrabold text-lg mb-4">{step}</div>
        ...
        <section>
          <div className="flex gap-3 mb-6">
            {['men', 'women'].map((g) => (
              <button
                key={g}
                onClick={() => setGender(g)}
                className={`px-5 py-2 rounded-full font-semibold text-sm transition-colors ${gender === g ? 'bg-dark text-white' : 'bg-white text-dark border border-gray-200'}`}
              >
                {g === 'men' ? 'Men' : 'Women'}
              </button>
            ))}
              ...
              <thead>
                <tr className="bg-dark text-white">
                  {Object.keys(SIZES).map((k) => (
                    <th key={k} className="py-3 px-4 font-semibold text-center">{k}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
          ...
          <p className="text-xs text-gray-400 mt-3">* Women's sizes are typically 1.5 sizes smaller than men's US sizing.</p>
        </section>

        <div className="card p-6 bg-brand/5 border border-brand/20">
          <h3 className="font-bold text-dark mb-1">Still unsure?</h3>
          <p className="text-gray-600 text-sm">
            Contact our team at <a href="mailto:support@sneakara.com" className="text-brand hover:underline">support@sneakara.com</a> and
