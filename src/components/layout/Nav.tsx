            ...
            <nav className="hidden md:flex items-center gap-8" aria-label="Main navigation">
              {NAV_LINKS.map((link) => (
                <button
                  ...
                  className={`text-[11px] font-semibold tracking-[0.2em] uppercase transition-colors duration-200 ${isActive(link.path)
                    ? 'text-white'
                    : link.highlight
              ...
              <nav className="flex-1" aria-label="Mobile navigation">
                {NAV_LINKS.map((link, i) => (
                  <button
                    ...
                    className={`flex items-end justify-between w-full py-6 border-b border-white/10 text-left transition-colors ${isActive(link.path) ? 'text-white' : link.highlight ? 'text-red-400' : 'text-white/70 hover:text-white'
                      }`}
                  >
                    <span
